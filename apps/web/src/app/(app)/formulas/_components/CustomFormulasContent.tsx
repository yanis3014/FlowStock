...
const DOC_EXAMPLES = [
  { expr: '(PRIX_VENTE - PRIX_ACHAT) * QUANTITE', desc: 'B뿯½n뿯½fice total en stock' },
...
export function CustomFormulasContent() {
  ...
  const handlePreview = useCallback(async () => {
    ...
    try {
      ...
      setEditorError(e instanceof Error ? e.message : 'Erreur pr뿯½visualisation');
    } finally {
      setPreviewing(false);
    }
  ...
  const tabs: { id: TabId; label: string }[] = [
    { id: 'editor', label: '뿯½diteur' },
    { id: 'library', label: 'Biblioth뿯½que' },
    ...
    <div className="space-y-6" role="region" aria-label="Formules personnalis├뿯½es">
      ...
      >
        ...
        <div className="mt-4 rounded-md border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800">
          <strong>Note IF() :</strong> IF 뿯½value les deux branches. Pour 뿯½viter les divisions par z뿯½ro, pr뿯½f뿯½rez{' '}
          <code>STOCK_ACTUEL / MAX(CONSOMMATION_MOYENNE, 0.001)</code> 뿯½ <code>IF(CONSO &gt; 0, STOCK/CONSO, 0)</code>.
        </div>
      </div>
    </div>
  );
