...
- **`npm run build` for the API has a pre-existing TypeScript rootDir error.** Dev mode (`npm run dev` / `ts-node-dev --transpile-only`) and tests (`jest` / `ts-jest`) work fine. Only production `tsc` build is affected.
